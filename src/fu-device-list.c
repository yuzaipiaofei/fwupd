...
fu_device_list_find_by_platform_id (FuDeviceList *self, const gchar *platform_id)
{
	...
	for (guint i = 0; i < self->devices->len; i++) {
		...
		if (item->device_old == NULL)
			continue;
		if (g_strcmp0 (fu_device_get_platform_id (item->device_old), platform_id) == 0)
			return item;
	}
	return NULL;
}
...
fu_device_list_remove (FuDeviceList *self, FuDevice *device)
{
	...
	if (fu_device_get_remove_delay (item->device) > 0) {
		...
		/* we can't do an update with an unconnected device */
...
fu_device_list_add (FuDeviceList *self, FuDevice *device)
{
	...
	item = fu_device_list_find_by_platform_id (self, fu_device_get_platform_id (device));
	if (item != NULL &&
	    fu_device_has_flag (item->device, FWUPD_DEVICE_FLAG_WAIT_FOR_REPLUG_AUTO)) {
	...
	/* allow the user to plug the a compatible device into a different port
	 * and for it to still be marked as the same device */
