...
GPtrArray *
dfu_chunked_new_from_bytes (GBytes *blob, guint32 addr_start, guint32 page_sz, guint32 packet_sz)
{
	gsize sz;
	const guint8 *data = g_bytes_get_data (blob, &sz);
	return dfu_chunked_new (data, (guint32) sz, addr_start, page_sz, packet_sz);
}
