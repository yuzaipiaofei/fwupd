 ...
 **/
DfuFirmwareFormat
dfu_firmware_detect_wac (GBytes *bytes)
{
	guint8 *data;
	gsize len;
...
/**
 * dfu_firmware_to_wac: (skip)
 * @firmware: a #DfuFirmware
 * @error: a #GError, or %NULL
 *
 * Packs a .wac firmware
 *
 * Returns: (transfer full): the packed data
 **/
GBytes *
dfu_firmware_to_wac (DfuFirmware *firmware, GError **error)
{
	g_set_error_literal (error,
			     FWUPD_ERROR,
			     FWUPD_ERROR_NOT_SUPPORTED,
			     "Wacom S-record export functionality missing");
	return FALSE;
}
 ...
 **/
gboolean
dfu_firmware_from_wac (DfuFirmware *firmware,
			 GBytes *bytes,
			 ...
			 GError **error)
{
	...
	for (guint i = 0; lines[i] != NULL; i++) {
		...
		if (g_strcmp0 (cmd, "S7") == 0) {
			...
			if (!dfu_image_from_srec (image, blob, flags, error))
				return FALSE;
	...
	dfu_firmware_set_format (firmware, DFU_FIRMWARE_FORMAT_WAC);
	return TRUE;
}
